...
function doPost(e) {
  ...
  /* 解除密碼鎖定：持有與 GAS 保管相同的 GitHub token 即可解鎖。
     不用 UrlFetchApp 驗證（該權限範圍此專案未授權，呼叫會直接失敗）。 */
  if (action === 'clearAdminLock') {
    var _t = String(p.ghtoken || '').trim();
    var _stored = PropertiesService.getScriptProperties().getProperty('GH_PAT') || '';
    if (!_t) return _jsonOut({ ok:false, error:'notoken' }, cb);
    if (!_stored || _t !== _stored) return _jsonOut({ ok:false, error:'badtoken' }, cb);
