// Certificate Generator v0.1.0
// 證書產生器 — 單面，A4 直/橫，文字 + 自動編號 + 圖片欄位（印章/簽名/浮水印），直橫書切換

function makeField(over) {
  return Object.assign({
    id: 'f_' + Math.random().toString(36).slice(2,8),
    label: '欄位',
    kind: 'text',          // 'text' | 'auto' | 'image'
    template: '',
    format: '#{n}',
    start: 1,
    padding: 0,
    ...
    // image kind:
    imageSrc: '',          // 固定圖 base64 data URL（印章 / 簽名 / 浮水印）
    imageTemplate: '',     // 從名單欄位拉圖：例 {photo} 或 {章}
    imageW: 30, imageH: 30,
    imageOpacity: 1
  }, over || {});
}
...
const state = {
  cardW: 210,          // mm (A4 portrait width)
  cardH: 297,          // mm (A4 portrait height)
  ...
  selectedFieldId: '',
  fields: [
    makeField({ id:'f_org',    label:'頒發單位', kind:'text', template:'{單位}',
                size:18, x:105, y:50,  align:'center', weight:700, lineHeight:1.2,
                font:'NotoSerifTC' }),
    makeField({ id:'f_title',  label:'稱號',     kind:'text', template:'獎  狀',
                size:48, x:105, y:90,  align:'center', weight:700, letterSpacing:0.3,
                font:'NotoSerifTC' }),
    makeField({ id:'f_name',   label:'姓名',     kind:'text', template:'{姓名}',
                size:36, x:105, y:135, align:'center', weight:700, letterSpacing:0.1,
                autoFit:true, maxFitW:160, font:'NotoSerifTC' }),
    makeField({ id:'f_body',   label:'事由',     kind:'text',
                template:'表現優異，特頒此狀，以資鼓勵。',
                size:16, x:105, y:180, align:'center', weight:400, lineHeight:1.6,
                autoFit:true, maxFitW:170, font:'NotoSerifTC' }),
    makeField({ id:'f_signer', label:'頒發人',   kind:'text', template:'{頒發人}',
                size:14, x:155, y:240, align:'center', weight:400 }),
    makeField({ id:'f_date',   label:'日期',     kind:'text', template:'中華民國 {日期}',
                size:12, x:105, y:265, align:'center', weight:400 }),
    makeField({ id:'f_no',     label:'字號',     kind:'auto', format:'字號 {n}',
                start:1, padding:4,
                size:10, x:30, y:280, align:'left', weight:400, color:'#666' })
  ],
...
function buildFieldEditors() {
  ...
  state.fields.forEach(f => {
    ...
    div.innerHTML = `
      <div class="row">
        ...
        <label>類型
          <select data-k="kind">
            ...
            <option value="image">圖片（印章 / 簽名 / 浮水印）</option>
          </select>
        </label>
      </div>
      ...
      <div class="img-only" style="display:none;">
        <label class="file-btn img-pick" style="margin-top:6px">
          🖼️ 上傳圖片（印章 / 簽名）
          <input type="file" class="img-file" accept="image/*">
        </label>
        <label>或從名單欄位拉圖<input type="text" data-k="imageTemplate" placeholder="例：{photo}"></label>
        <div class="row">
          <label>寬 (mm)<input type="number" data-k="imageW" min="1" step="1"></label>
          <label>高 (mm)<input type="number" data-k="imageH" min="1" step="1"></label>
          <label>透明度<input type="number" data-k="imageOpacity" min="0" max="1" step="0.05"></label>
        </div>
        <div class="img-preview" style="margin:6px 0; padding:6px; background:#f7f8fa; border-radius:4px; text-align:center; max-height:120px; overflow:hidden;"></div>
      </div>
      <div class="row">
        <label>字型
          <select data-k="font">
      ...
      <div class="row">
        <label>對齊
          <select data-k="align">
            <option value="left">靠左 / 左上</option>
            <option value="center">置中</option>
            <option value="right">靠右 / 右上</option>
          </select>
        </label>
        <label>旋轉 (度)<input type="number" data-k="rotation" step="1" min="-180" max="180"></label>
      </div>
      <div class="row">
        <label><input type="checkbox" data-k="vertical"> 直書（中式縱向書寫）</label>
      </div>
      <div class="row">
    ...
    // toggle auto-only / img-only blocks based on kind
    const autoRow = div.querySelector('.auto-only');
    const imgBlock = div.querySelector('.img-only');
    const imgPreview = div.querySelector('.img-preview');
    const textOnlyRows = []; // stash refs to text-only rows so we can hide them when kind='image'
    const refreshKindUI = () => {
      if (autoRow) autoRow.style.display = (f.kind === 'auto') ? '' : 'none';
      if (imgBlock) imgBlock.style.display = (f.kind === 'image') ? '' : 'none';
      // hide font/size/color/letterSpacing/lineHeight/autoFit/stroke/shadow/template rows for image kind
      div.querySelectorAll('[data-text-only]').forEach(el => {
        el.style.display = (f.kind === 'image') ? 'none' : '';
      });
      if (imgPreview) {
        imgPreview.innerHTML = '';
        if (f.kind === 'image' && f.imageSrc) {
          const im = document.createElement('img');
          im.src = f.imageSrc;
          im.style.cssText = 'max-width:100%; max-height:100px;';
          imgPreview.appendChild(im);
        }
      }
    };
    // mark all text-only rows
    [...div.children].forEach(child => {
      if (child === autoRow || child === imgBlock) return;
      // skip first 2 rows (label/kind, auto-only is handled)
      // Mark rows by index: keep label-row and kind-row always; everything else is text-only
      if (child.classList && child.classList.contains('row')) {
        // Identify label/kind row: contains data-k="label" or data-k="kind"
        const hasLabel = child.querySelector('[data-k="label"], [data-k="kind"]');
        if (!hasLabel) child.setAttribute('data-text-only', '1');
      }
    });
    // Template textarea is text-only
    const tplLabel = div.querySelector('textarea[data-k="template"]')?.parentElement;
    if (tplLabel) tplLabel.setAttribute('data-text-only', '1');
    refreshKindUI();
    // Image upload
    const imgFile = div.querySelector('.img-file');
...
// === Image cache (HTMLImageElement keyed by data URL) ===
...
function drawImageField(tctx, f, row, pxPerMm) {
  // 決定來源：先看 imageTemplate（從 row 抓網址），否則 imageSrc
  let src = '';
  if (f.imageTemplate && row) {
    src = applyTemplate(f.imageTemplate, row).trim();
  }
  if (!src) src = f.imageSrc || '';
  if (!src) return;
  ...
  tctx.globalAlpha = (typeof f.imageOpacity === 'number') ? f.imageOpacity : 1;
  const w = (f.imageW || 30) * pxPerMm;
  const h = (f.imageH || 30) * pxPerMm;
  // 對齊：以 (x,y) 為錨點，依 align 解釋（left=左上、center=中央、right=右上）
  let dx = 0, dy = 0;
  if (f.align === 'center') dx = -w / 2;
  else if (f.align === 'right') dx = -w;
...
function drawFaceText(tctx, row, pxPerMm, rowIdx) {
  state.fields.forEach(f => {
    if (f.kind === 'image') {
      drawImageField(tctx, f, row, pxPerMm);
      return;
    }
...
function drawTextBlock(tctx, text, f, pxPerMm) {
  // 直書分支
  if (f.vertical) return drawTextBlockVertical(tctx, text, f, pxPerMm);
  let sizePt = f.size;
...
function drawTextBlockVertical(tctx, text, f, pxPerMm) {
  // 直書：每行(行=column)由上而下；多行從右至左（傳統中文閱讀）
  const sizePt = f.size;
  const px = sizePt * pxPerMm * (25.4 / 72);
  const lines = String(text).split(/\r?\n/);
  const charH = px * (f.lineHeight || 1.0);     // 字距（垂直方向）
  const colW  = px * 1.5;                        // 行距（水平方向）
  tctx.save();
  tctx.translate(f.x * pxPerMm, f.y * pxPerMm);
  if (f.rotation) tctx.rotate((f.rotation * Math.PI) / 180);
  tctx.font = `${f.weight} ${px}px "${f.font}", sans-serif`;
  ...
  // 計算總高（最長行）以便對齊
  const maxLineLen = Math.max(...lines.map(l => Array.from(l).length), 1);
  const totalH = charH * maxLineLen;
  let yStart = 0;
  if (f.align === 'center') yStart = -totalH / 2;
  else if (f.align === 'right') yStart = -totalH;
  // 行從右至左
  lines.forEach((line, lineIdx) => {
    const colX = -lineIdx * colW; // 第一行為 0，往左遞減
    const chars = Array.from(line);
    if (f.stroke && f.stroke > 0) {
      tctx.lineJoin = 'round';
      tctx.lineWidth = f.stroke * pxPerMm * 0.3;
      tctx.strokeStyle = f.strokeColor || '#fff';
      chars.forEach((ch, i) => {
        tctx.strokeText(ch, colX, yStart + (i + 0.5) * charH);
      });
      tctx.shadowColor = 'transparent';
    }
    tctx.fillStyle = f.color;
    chars.forEach((ch, i) => {
      tctx.fillText(ch, colX, yStart + (i + 0.5) * charH);
    });
  });
  tctx.restore();
}
...
async function saveImpAsPdf() {
  ...
  await saveSingleFile(blob, autoFilename('證書拼版', '.pdf'), 'application/pdf', '.pdf', 'PDF 檔');
}
...
async function saveImpAsPng() {
  ...
  if (files.length === 1) {
    await saveSingleFile(files[0].blob, autoFilename('證書拼版', '.png'),
      'image/png', '.png', 'PNG 圖片');
  } else {
    // 多頁：把檔名前綴也改成自動命名
    const prefix = autoFilename('證書拼版', '');
...
async function saveEachAsPdf() {
  ...
  const blob = doc.output('blob');
  await saveSingleFile(blob, autoFilename('證書單張', '.pdf'), 'application/pdf', '.pdf', 'PDF 檔');
}
...
const LS_KEY = 'certificateState_v1';
let _saveT = 0;
function autosave() {
  clearTimeout(_saveT);
  _saveT = setTimeout(() => {
...
async function saveProject() {
  ...
  await saveSingleFile(blob, autoFilename('證書專案', '.json'),
    'application/json', '.json', '證書專案');
}

async function saveTemplate() {
  const bundle = buildBundle(false);
  const blob = new Blob([JSON.stringify(bundle)], { type: 'application/json' });
  await saveSingleFile(blob, autoFilename('證書模板', '.template.json'),
    'application/json', '.json', '證書模板');
}
