...
const state = {
  ...
  selectedField: 'name',
  fields: {
    org:   { font: 'NotoSansTC', weight: 400, size: 12, color: '#0a2a66',
             x: 8, y: 12, align: 'left', letterSpacing: 0, lineHeight: 1.3,
             rotation: 0, template: '{單位}',
             autoFit: false, maxFitW: 130, stroke: 0, strokeColor: '#ffffff',
             shadow: false, shadowColor: 'rgba(0,0,0,0.4)', shadowBlur: 2 },
    name:  { font: 'NotoSansTC', weight: 700, size: 36, color: '#0a2a66',
             x: 74, y: 33, align: 'center', letterSpacing: 0.05, lineHeight: 1.15,
             rotation: 0, template: '{姓名}',
             autoFit: true, maxFitW: 130, stroke: 0, strokeColor: '#ffffff',
             shadow: false, shadowColor: 'rgba(0,0,0,0.4)', shadowBlur: 2 },
    title: { font: 'NotoSansTC', weight: 400, size: 14, color: '#0a2a66',
             x: 140, y: 48, align: 'right', letterSpacing: 0, lineHeight: 1.3,
...
async function saveImpAsPdf() {
  ...
  await saveSingleFile(blob, autoFilename('桌牌拼版', '.pdf'), 'application/pdf', '.pdf', 'PDF 檔');
}
...
async function saveImpAsPng() {
  ...
  if (files.length === 1) {
    await saveSingleFile(files[0].blob, autoFilename('桌牌拼版', '.png'),
      'image/png', '.png', 'PNG 圖片');
  } else {
    // 多頁：把檔名前綴也改成自動命名
    const prefix = autoFilename('桌牌拼版', '');
...
async function saveEachAsPdf() {
  ...
  const blob = doc.output('blob');
  await saveSingleFile(blob, autoFilename('桌牌單張', '.pdf'), 'application/pdf', '.pdf', 'PDF 檔');
}
...
async function saveProject() {
  ...
  await saveSingleFile(blob, autoFilename('桌牌專案', '.json'),
    'application/json', '.json', '桌牌專案');
}

async function saveTemplate() {
  const bundle = buildBundle(false);
  const blob = new Blob([JSON.stringify(bundle)], { type: 'application/json' });
  await saveSingleFile(blob, autoFilename('桌牌模板', '.template.json'),
    'application/json', '.json', '桌牌模板');
}
